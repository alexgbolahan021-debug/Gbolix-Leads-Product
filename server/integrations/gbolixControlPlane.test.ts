...
const mocks = vi.hoisted(() => ({ ingestOpenStreetMapDiscovery: vi.fn(), discover: vi.fn() }));

vi.mock("../leadDb", () => ({
  authorizeWorkspaceExportDownload: vi.fn(),
  createWorkspaceRequestExport: vi.fn(),
  getWorkspaceRequestResults: vi.fn(),
  ingestOpenStreetMapDiscovery: mocks.ingestOpenStreetMapDiscovery,
...
afterEach(() => {
  mocks.ingestOpenStreetMapDiscovery.mockReset();
...
describe("signed discovery intake", () => {
  it("forwards confirmed chat constraints into the discovery persistence metadata", async () => {
    ...
    mocks.ingestOpenStreetMapDiscovery.mockResolvedValue({ jobId: "job_1", createdCount: 0, duplicateCount: 0 });
    ...
    try {
      ...
      expect(mocks.ingestOpenStreetMapDiscovery).toHaveBeenCalledWith(expect.objectContaining({ requestMetadata: expect.objectContaining({ keywords: ["website", "automation"], city: "Lagos, Nigeria", requestedLimit: 5 }) }));
    } finally {
